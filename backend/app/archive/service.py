...
from app.db.models import ArchiveExport, Article, Source
...
async def _save_article_markdown(
    ...
    *,
    ...
    if record is None:
        return None
    article, source_name, source_url = record
    ...
    try:
        export = await export_article(
            session,
            article,
            ArchiveMetadata(source_name=source_name, source_url=source_url),
            writer,
        )
